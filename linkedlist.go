...
func mergeMoreList(list []*TwitterNode, start, end int) []int {
	if len(list) < 1 || start > end {
		return nil
	}
	if start == end {
		return mergeSortList(list[start], nil)
	}
	middle := start + (end-start)/2
	return mergeSortList(mergeMoreList(list, start, middle), mergeMoreList(list, middle+1, end))
}

func mergeSortList(l1, l2 *TwitterNode) []int {
	head := &TwitterNode{}
	h := head
	for l1 != nil && l2 != nil {
		if l1.Time < l2.Time {
...
func (t *Twitter) Follow(followerId int, followeeId int) {
	...
	t.followeeList[followerId][followerId] = followeeId
	return
}
