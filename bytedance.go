...
func main() {
	...
	r := mergeInBetween1(list1, 0, 1, list2)
	PrintList(r)
}
...
func mergeInBetween1(list1 *ListNode, a int, b int, list2 *ListNode) *ListNode {
	...
	pre, slow, fast, i := dummyHead, dummyHead, dummyHead, a
	for i < b && fast != nil {
		fast = fast.Next
		i++
	}
...
func mergeInBetween(list1 *ListNode, a int, b int, list2 *ListNode) *ListNode {
	...
	pre, slow, fast, last, len := list1, list1, list1, list1, 0
	for last.Next != nil {
		last = last.Next
		len++
	}
...
func mergeKLists12(lists []*ListNode) *ListNode {
	if len(lists) < 1 {
		return nil
	}
...
func merge11(lists []*ListNode, start, end int) *ListNode {
	if start == end {
		return lists[start]
	}
	if start > end {
	...
	return mergeSort11(merge11(lists, start, middle), merge11(lists, middle+1, end))
}

func mergeSort11(list1, list2 *ListNode) *ListNode {
	dummyHead := &ListNode{}
	h := dummyHead
...
func deleteDuplicates1(head *ListNode) *ListNode {
	...
	newHead := &ListNode{Val: head.Val - 1, Next: head}
	cur, next, val := newHead, newHead.Next, 0
...
func findKthLargest123(nums []int, k int) int {
	...
	l, r, sk := 0, len(nums)-1, len(nums)-k
	for l < r {
		p := getPartitionw(nums, l, r)
		if p == sk {
			return nums[p]
		} else if p > sk {
			r = p - 1
...
func getPartitionw(nums []int, l int, r int) int {
	p := nums[l]
	for l < r {
		for l < r && p <= nums[r] {
			r--
...
func mergeLinkList1122(head1 *ListNode, head2 *ListNode) *ListNode {
...
func reverseBetween(head *ListNode, m, n int) *ListNode {
	if m == 1 {
		return reverseM(head, n)
	}
	head.Next = reverseBetween(head.Next, m-1, n-1)
	return head
}

func reverseM(head *ListNode, n int) *ListNode {
	if n == 1 {
		next = head.Next
		return head
	}
	last := reverseM(head.Next, n-1)
	head.Next.Next = head
	head.Next = next
	return last
}
